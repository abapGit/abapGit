...
CLASS zcl_abapgit_password_dialog IMPLEMENTATION.
  ...
  METHOD popup.
    ...
    DATA lv_default_git_uname TYPE string.
    DATA lv_default_git_email TYPE string.

    lv_default_git_uname = zcl_abapgit_persist_factory=>get_settings( )->read( )->get_default_git_uname( ).
    lv_default_git_email = zcl_abapgit_persist_factory=>get_settings( )->read( )->get_default_git_email( ).

    IF lv_default_git_uname IS NOT INITIAL.
      cv_user = lv_default_git_uname.
    ELSEIF lv_default_git_email IS NOT INITIAL.
      cv_user = lv_default_git_email.
    ENDIF.
