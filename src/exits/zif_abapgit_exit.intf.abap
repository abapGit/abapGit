INTERFACE zif_abapgit_exit PUBLIC.
  ...
  METHODS adjust_display_commit_url
    ...
    CHANGING  cv_display_url TYPE csequence
    RAISING   zcx_abapgit_exception.

  METHODS adjust_display_filename
    IMPORTING is_repo_meta       TYPE zif_abapgit_persistence=>ty_repo
  ...
  METHODS change_local_host
    CHANGING ct_hosts TYPE zif_abapgit_definitions=>ty_string_tt.

  METHODS change_max_parallel_processes
    IMPORTING iv_package       TYPE devclass
    CHANGING  cv_max_processes TYPE i.
  ...
  METHODS change_rfc_server_group
    CHANGING cv_group TYPE rzlli_apcl.

  METHODS change_supported_data_objects
    CHANGING ct_objects TYPE zif_abapgit_data_supporter=>ty_objects.

  METHODS change_supported_object_types
    CHANGING ct_types TYPE ty_object_types.
  ...
  METHODS deserialize_postprocess
    IMPORTING is_step TYPE zif_abapgit_objects=>ty_step_data
              ii_log  TYPE REF TO zif_abapgit_log.

  METHODS determine_transport_request
    IMPORTING ii_repo              TYPE REF TO zif_abapgit_repo
              iv_transport_type    TYPE zif_abapgit_definitions=>ty_transport_type
    CHANGING  cv_transport_request TYPE trkorr.
  ...
  METHODS get_ci_tests
    ...
    CHANGING  ct_ci_repos TYPE ty_ci_repos.

  METHODS get_ssl_id
    RETURNING VALUE(rv_ssl_id) TYPE ssfapplssl.
  ...
  METHODS on_event
    IMPORTING ii_event          TYPE REF TO zif_abapgit_gui_event
    RETURNING VALUE(rs_handled) TYPE zif_abapgit_gui_event_handler=>ty_handling_result
    RAISING   zcx_abapgit_exception.

  METHODS pre_calculate_repo_status
    IMPORTING is_repo_meta TYPE zif_abapgit_persistence=>ty_repo
    CHANGING  ct_local     TYPE zif_abapgit_definitions=>ty_files_item_tt
  ...
  METHODS serialize_postprocess
    IMPORTING iv_package TYPE devclass
              ii_log     TYPE REF TO zif_abapgit_log
    CHANGING  ct_files   TYPE zif_abapgit_definitions=>ty_files_item_tt.
  ...
  METHODS change_committer_info
    IMPORTING iv_repo_url TYPE csequence
    CHANGING  cv_name     TYPE csequence
