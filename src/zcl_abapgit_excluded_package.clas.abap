...
CLASS zcl_abapgit_excluded_package IMPLEMENTATION.
  ...
  METHOD get_packages.
    ...
    FIELD-SYMBOLS: <lv_subpackage> TYPE ty_excluded_packages.

    lt_packages = to_table( iv_excluded_packages ).

    WHILE lt_r_packages IS NOT INITIAL.

      CLEAR: lt_r_packages.

      LOOP AT lt_packages ASSIGNING <lv_subpackage>.

        ls_r_packages-sign   = 'I'.
        ls_r_packages-option = 'EQ'.
        ls_r_packages-low    = <lv_subpackage>.

        APPEND: ls_r_packages TO lt_r_packages,
                ls_r_packages TO rt_r_packages.
  ...
  METHOD to_table.
    ...
    DELETE rt_packages WHERE table_line CO ' _0'.
    SORT rt_packages.
