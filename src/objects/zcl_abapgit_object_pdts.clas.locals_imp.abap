...
CLASS lcl_task_definition DEFINITION
  ...
  PUBLIC SECTION.
    ...
    ALIASES: clear_origin_data FOR lif_task_definition~clear_origin_data,
             ...
             save FOR lif_task_definition~save,
             change_wi_text FOR lif_task_definition~change_wi_text,
             change_method FOR lif_task_definition~change_method,
             change_start_events FOR lif_task_definition~change_start_events,
             change_terminating_events FOR lif_task_definition~change_terminating_events,
             change_text FOR lif_task_definition~change_text.

    CLASS-METHODS load IMPORTING iv_objid         TYPE hrobject-objid
                       RETURNING VALUE(ri_result) TYPE REF TO lif_task_definition
                       RAISING   zcx_abapgit_exception.

    CLASS-METHODS create_new IMPORTING is_task_data     TYPE lif_task_definition=>ty_task_data
                             RETURNING VALUE(ri_result) TYPE REF TO lif_task_definition
                             RAISING   zcx_abapgit_exception.
