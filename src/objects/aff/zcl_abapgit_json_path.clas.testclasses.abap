...
CLASS ltcl_json_path IMPLEMENTATION.
  ...
  METHOD serialize.
    DATA(lo_ajson) = zcl_abapgit_ajson=>new( iv_keep_item_order = abap_true
    )->set( iv_path = '/'
            iv_val  = data
    ...
    DATA(cut) = NEW zcl_abapgit_json_path( ).
    result = cut->serialize( lo_ajson->stringify( ) ).
  ENDMETHOD.


  METHOD flat_structure.

    DATA data TYPE zif_abapgit_aff_intf_v1=>ty_main.
    data-header-description = 'Interface zum BAdI: BADI_TADIR_CHANGED'.

    DATA(act) = serialize( data ).
    DATA(exp) = VALUE string_table( ( `$.header.description=Interface zum BAdI: BADI_TADIR_CHANGED` ) ).
  ...
  METHOD array.
    ...
    DATA data TYPE zif_abapgit_aff_intf_v1=>ty_main.
    data-header-description = 'Interface zum BAdI: BADI_TADIR_CHANGED'.
    data-descriptions-methods = VALUE #( ( name = 'AFTER_TADIR_CHANGED'
