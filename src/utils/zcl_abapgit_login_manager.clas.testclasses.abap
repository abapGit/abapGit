...
CLASS ltcl_login_manager IMPLEMENTATION.
  ...
  METHOD auto_login_background_mode.
    ...
    "and BOT users + access tokens per project (for example in gitlab) have
    "been generated to be able to auto commit in abapGit background mode
    ...
    CONSTANTS:
      lc_project_a TYPE string VALUE 'https://local.gitlab.com/projectA.git',
      lc_project_b TYPE string VALUE 'https://local.gitlab.com/projectB.git'.

    CONSTANTS:
      lc_username_a TYPE string VALUE 'project_a_bot',
      lc_password_a TYPE string VALUE 'Op3cSesasdfPOpojasdf',
      lc_username_b TYPE string VALUE 'project_b_bot',
      lc_password_b TYPE string VALUE 'loinonwasdfPOpvunwef'.

    "WHEN

    zcl_abapgit_login_manager=>set(  iv_uri      = lc_project_a
                                     iv_username = lc_username_a
                                     iv_password = lc_password_a ).


    zcl_abapgit_login_manager=>set(  iv_uri      = lc_project_b
                                     iv_username = lc_username_b
                                     iv_password = lc_password_b ).
