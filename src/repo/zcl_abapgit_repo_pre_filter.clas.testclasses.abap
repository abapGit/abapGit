...
CLASS ltcl_adjust_filter IMPLEMENTATION.
  ...
  METHOD adjust_local_filter_lang.
    ...
    "Has only 40 Chars so the R will be cut
    ls_filter_adj-obj_name = 'CL_GUI_ALV_GRID               CONSTRUCTO'.
    INSERT ls_filter_adj INTO TABLE lt_filter_adj.
    ...
    ls_filter_adj-pgmid = 'R3TR'.
    ls_filter_adj-object = 'DEVC'.
    ls_filter_adj-obj_name = 'SLIS'.
    INSERT ls_filter_adj INTO TABLE lt_filter_adj.
    ...
    lv_package = 'SLIS'.
    TRY.
        lt_filter_adj_res = mo_cut->adjust_local_filter(
                                                          iv_package     = lv_package
                                                          it_e071_filter = lt_e071_filter ).

        cl_abap_unit_assert=>assert_equals( exp = lt_filter_adj
